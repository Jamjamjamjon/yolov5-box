...
#-----------------------------------------------------------------------
#    other backbone modules
#-----------------------------------------------------------------------
from models.PP_LCNet.PP_LCNet import *          # pp-LCNet modules
from models.mobileNet.mobileNet import *        # mobileNet modules
from models.shuffleNet.shuffleNet import *      # shuffleNet modules
from models.RepVGG.RepVGG import *              # RepVGG modules
...
class Detect(nn.Module):
    ...
    permute = True     # to export ONNX with / without permute layer @jamjon
    ...
    def forward(self, x):
        ...
        #------------------------------------------------
        #  to export ONNX without permute layer @jamjon
        #------------------------------------------------
        if not self.permute:
            # rich.print(f"[gold1 bold]==>output ONNX with no permute, outputs shape like:[1, 255, xx, xx]")
            for i in range(self.nl):
                x[i] = self.m[i](x[i])  # conv
                # rich.print("x[i] shape: ", x[i].shape)
            self.training = True
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, Shuffle_Block, conv_bn_relu_maxpool, DWConvblock, stem, 
                 Light_C3, ADD, RepVGGBlock, SEBlock, Conv3BN, InvertedResidual]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', action='store_true', help='print model info')
