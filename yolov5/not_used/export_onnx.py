...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    if spp_optimize in ["yes", "y", "ok"]:
        ...
        rich.print("  ==> use param [underline bold red]--spp_optimize=no[/underline bold red] to cancel.")
    if not_permute in ["yes", "y", "ok"]:
        rich.print(f"[gold1 bold]  ==> ONNX export with no permute layer, shape like: [1, 255, xx, xx]")
        rich.print("  ==> use param [underline bold red]--not_permute=no[/underline bold red] to cancel.")
    #------------------------------
    ...
    for k, m in model.named_modules():
        ...
        # if opt.no_permute:
        if not_permute in ["yes", "y", "ok"]:
            model.model[-1].not_permute = True
    ...
    rich.print(f"[bold green]Results saved to [bold underline white]{file.parent.resolve()}")
...
def parse_opt():
    ...
    parser.add_argument('--not-permute', type=str, default='yes', help='export ONNX without permute layer')   
